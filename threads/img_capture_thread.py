...
class ImgCapture_Thread(QThread):  # 用于扫描 这个类可能用于在一个新的线程中执行图像捕获任务。 信号可以用于将结果从线程发送到主线程。
    ...
    def run(self):  # 重写QThread的run方法，run方法在启动线程（start方法）后会被调用，用于执行线程的任务。
        ...
        while True:
            ...
            img1_save_path = os.path.join(self.savapath, str(scanning_index_now), str(1) + '.png')
            cv2.imwrite(img1_save_path, img1)
            # cv2.imwrite（filename ，img） filename 参数是你想要保存的文件的名称，包括文件路径和扩展名。img 参数是你想要保存的图像
            self.signal_img1_set.emit(img1_save_path, img1)  # 当调用emit()方法时，信号对象会发出信号，并将img1作为参数传递给连接到该信号的槽（slot）函数。槽函数是与信号相关联的函数，它会在信号发出时被调用。
            
            com_arduino.write('2\r\n'.encode())

            img2 = grab_image_save()
            img2_save_path = os.path.join(self.savapath, str(scanning_index_now), str(2) + '.png')
