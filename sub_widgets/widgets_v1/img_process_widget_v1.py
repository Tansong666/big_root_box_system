...
# from threads.img_preprocess_thread import PreProcessThread
...
class ImgProcessWidget(QWidget):
    def __init__(self, parent=None):
        ...
        # self.preprocess_thread.log_signal.connect(self.update_textBrowser)
        # self.preprocess_thread.concat_signal.connect(self.update_process_view)
        # self.preprocess_thread.seg_signal.connect(self.update_process_view)
    ...
    def on_model_loaded(self, model):
        # self.preprocess_thread.model = model
    ...
    def auto_prepocess_thread(self,image_path1,image_path2,img1,img2):
        ...
        # 创建线程并连接信号，可以实现连续的信号传递
        if self.preprocess_thread.isRunning():
            self.preprocess_thread.wait()  # 等待线程终止
            self.update_textBrowser('等待线程中')
            self.preprocess_thread = PreProcessThread()
            self.preprocess_thread.log_signal.connect(self.update_textBrowser)
            self.preprocess_thread.concat_signal.connect(self.update_process_view)
            self.preprocess_thread.seg_signal.connect(self.update_process_view)
        # 图像拼接+分割线程
        # 打印所有参数
        # if self.ui.lbl_dataDirShow.text() == "None":
        #     self.preprocess_thread.input_path = None
        #     print('input_path is str None')
        # else:
        #     self.preprocess_thread.input_path = self.ui.lbl_dataDirShow.text()
        #     print(self.preprocess_thread.input_path)
        self.preprocess_thread.model = self.seg_model
        self.preprocess_thread.input_path = None
        self.preprocess_thread.img1 = img1
        self.preprocess_thread.img2 = img2
        self.preprocess_thread.img1_path = image_path1
        self.preprocess_thread.img2_path = image_path2
        self.preprocess_thread.concat_args = concat_args
        self.preprocess_thread.seg_args = seg_args
        ...
        # if self.preprocess_thread.isRunning():
        #     self.process_thread.wait()  # 等待线程终止
        #     self.update_textBrowser('等待线程中')
        self.preprocess_thread.start()

    def auto_postprocess_thread(self, img_file_path, img):
        sender = self.sender()  # 返回发送当前信号的对象（控件）
        # 从控件获取参数
        denoise_args = {}
        # if self.ui.lbl_dataDirShow.text() == "None":
        #     denoise_args['input_path'] = None
        #     print('input_path is str None')
        # else:
        #     denoise_args['input_path'] = self.ui.lbl_dataDirShow.text()
        ...
        inpaint_args = {}
        # inpaint_args['img_path'] = self.ui.lbl_dataDirShow.text()
        ...
        calculate_args = {}
        if sender == self.ui.btn_startDenoise or sender == self.ui.btn_startInpaint:
            calculate_args['is_calculate'] = False
        else:
            ...
            # calculate_args['img_path'] = self.ui.lbl_dataDirShow.text()
        ...
        # 更新线程参数
        # if self.ui.lbl_dataDirShow.text() == "None":
        #     self.process_thread.input_path = None
        #     print('input_path is str None')
        # else:
        #     self.process_thread.input_path = self.ui.lbl_dataDirShow.text()
        self.process_thread.input_path = None
    ...
    def init_ui(self):
        ...
        signals.img_preprocess_signal.connect(self.auto_prepocess_thread,Qt.QueuedConnection) # 全流程
        signals.img_postprocess_signal.connect(self.auto_postprocess_thread,Qt.QueuedConnection) # 全流程
